...
public class DateUtils {
    ...
    /**
     * @Author hld
     * @Description //获取下一个整点半小时，例如输入2019-07-16 12:07:00 输出2019-07-16 30:00:00
     * @Date 16:09 2019-7-15
     * @Param date 日期
     * @return java.lang.String
     **/
    public static Date nextHalfHour(Date date){
        ...
        if (minute > 30){
            halfDate = addDateMinutes(date,60-minute);  //下一个整点
            //halfDate = addDateHours(date,1);
        } else {
            halfDate = addDateMinutes(date,30-minute);  //下一个半点
        }
        halfDate = addDateSeconds(halfDate,0-second);   //秒取整
        return halfDate;
    }
     ...
     * @return true/false
     */
    public static boolean diffHalfHour(String beginTime,String endTime){
        Date addTime = addDateMinutes(stringToDate(beginTime),30);
        if (addTime.compareTo(stringToDate(endTime)) == 0){
            return true;
        }
        return false;
    }
    ...
    // 获取两个时间段之间所有的半点、整点时间(不包含结束时间)
    public static List<String> timeBetweenDate(String beginTime,String endTime) {
        List<String> times = new ArrayList<>();
        while (beginTime.compareTo(endTime) < 0){
            times.add(beginTime);
            beginTime = dateToString(addDateMinutes(stringToDate(beginTime),30));
        }
        return times;
    }

    //获取两个时间之间的时间差（分钟）
    public static long getHourbetweenDate(String beginTime,String endTime){
        Date beginDate = stringToDate(beginTime,DATE_PATTERN);
        Date endDate = stringToDate(endTime,DATE_PATTERN);
        //时间差（分钟）
        long min = (endDate.getTime()-beginDate.getTime())/(1000*60*60*24);
